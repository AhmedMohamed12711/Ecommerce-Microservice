...
public class BasketOrderingConsumerV2: IConsumer<BasketCheckoutEventV2>
{
    ...
    {
        using var scope = _logger.BeginScope("sonsuming basket checkout event  for {correlationid} with version 2", context.Message.CorrelationId);
        var cmd = _mapper.Map<CheckoutOrderCommandV2>(context.Message);
        var result = await _mediator.Send(cmd);
        _logger.LogInformation("Basket checkout event is completed version 2!!");
    }
}
