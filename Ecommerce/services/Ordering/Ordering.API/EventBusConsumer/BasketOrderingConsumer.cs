...
public class BasketOrderingConsumer : IConsumer<BasketCheckoutEvent>
{
    ...
    {
        using var scope = _logger.BeginScope("sonsuming basket checkout event  for {correlationid}", context.Message.CorrelationId);
        var cmd = _mapper.Map<CheckoutOrderCommand>(context.Message);
        var result=await _mediator.Send(cmd);
        _logger.LogInformation("Basket checkout event is completed !!");
    }
}
